...
```
```{r}
ggplot(data, aes(x = work)) + 
  geom_histogram()
```

```{r}
ggplot(data, aes(x = age, y = work)) +
  geom_point()
```
...
sd(data$work, na.rm = TRUE)
```

```{r}
# Standard deviation meetings
sd(data$meetings, na.rm = TRUE)
```
...
```{r}
se = sd(data$work, na.rm = TRUE)/sqrt(length(data$work))
se
```
